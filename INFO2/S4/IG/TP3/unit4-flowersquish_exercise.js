...
function fillScene() {
	...
	var petalLength = 120;
	var cylGeom = new THREE.CylinderGeometry( 15, 0, petalLength-10, 32 );
	...
	for (var i = 0; i < 24; i++){
		var clone = obj.clone();
		clone.rotateY((2*Math.PI/24)*(i+1));
		clone.rotateX(-2*Math.PI/24);
		clone.scale.set(1, 0.3, 1);
		flower.add(clone);
	}
	//flower.add(obj);
