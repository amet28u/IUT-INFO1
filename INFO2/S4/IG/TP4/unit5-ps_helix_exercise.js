...
function createHelix( material, radius, tube, radialSegments, tubularSegments, height, arc, clockwise )
{
	...
	for ( var i = 0; i <= arc*radialSegments ; i++ )
	{
		...
		// going from X to Z axis
		top2.set( radius * Math.cos( (i+1) * 2*Math.PI / radialSegments ),
		height * ((i+1)/(arc*radialSegments)) - height/2,
		sine_sign * radius * Math.sin( (i+1) * 2*Math.PI / radialSegments ) );

		var capsule;

		if (i == 0 || i == arc*radialSegments){
			capsule = createCapsule(material, tube, top2, top, tubularSegments, false, false);
		}
		else{
			capsule = createCapsule(material, tube, top2, top, tubularSegments, true, false);
		}
		helix.add(capsule);
	}
	///////////////
