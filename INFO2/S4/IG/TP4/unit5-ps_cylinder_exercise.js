...
function createCylinderFromEnds( material, radiusTop, radiusBottom, top, bottom, segmentsWidth, openEnded)
{
	...
	var length = Math.sqrt((top.x-bottom.x)*(top.x-bottom.x)+(top.y-bottom.y)*(top.y-bottom.y)+(top.z-bottom.z)*(top.z-bottom.z));
	var cylAxis = new THREE.Vector3(top.x-bottom.x, top.y-bottom.y, top.z-bottom.z);
	var center = new THREE.Vector3(bottom.x+(cylAxis.x/2), bottom.y+(cylAxis.y/2), bottom.z+(cylAxis.z/2));
	////////////////////
